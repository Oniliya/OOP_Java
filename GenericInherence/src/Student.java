...
//
/**
 * дженейрик класс студент 
 * тип T для передачи String
 * тип V для передачи Integer
 */
public class Student<T extends Comparable<T>,V> extends Person<T,V> implements Comparable<Student<T,V>> {

    private V id;
    /**
     * конструктор класса студент наследника персон
     *  
     * @param name имя (общее поле от класса Персон)
     * @param age возраст (общее поле от класса Персон)
